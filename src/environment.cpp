...
// void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    inputCloud, 0.2, Eigen::Vector4f (-5, -6, -3, 1), Eigen::Vector4f (30, 6, 0.0, 1));
    std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud =
    pointProcessorI->SegmentPlane (filterCloud, 100, 0.1);


    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters_filtered =
    pointProcessorI->Clustering (segmentCloud.first, 0.3, 100, 1600);
    int clusterId = 0;
...
void simpleHighway (pcl::visualization::PCLVisualizer::Ptr& viewer) {
    ...
    for (pcl::PointCloud<pcl::PointXYZ>::Ptr cluster : cloudClusters) {
        if (render_clusters) {
            ...
            "obstCloud" + std::to_string (clusterId), colors[clusterId]);
        }
        if (render_box) {
